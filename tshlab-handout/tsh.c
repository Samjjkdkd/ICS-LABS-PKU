...
eval(char *cmdline) 
{
    ...
    struct job_t *job;
    sigset_t mask_all, mask_three, prev_one, prev_hup, mask_hup;
    /*set masks*/
    sigfillset(&mask_all);

    sigemptyset(&prev_one);

    sigemptyset(&prev_hup);
    ...
    /*check if the job has been re-continued&print the job list*/
    if (tok.builtins == BUILTIN_JOBS){
        int fdout,status;
        if (tok.outfile == NULL)
            fdout = 1;
        else
            if((fdout = open(tok.outfile, O_RDWR|O_CREAT)) < 0)
                fprintf(stderr, "Couldn't open or create the file%s\n", tok.outfile);
    ...
    /*bg job or fg job
      bg_fg: 1 if cmdline reads fg
             2 if cmdline reads bg*/
    if(tok.builtins == BUILTIN_BG || tok.builtins == BUILTIN_FG){
        ...
        if (tok.argv[1] == NULL){
            fprintf(stderr, "The command %s needs a pid or %%jid\n", tok.argv[0]);
            return;
        }
        /*jid*/
        if (tok.argv[1][0] == '%'){
        ...
        /*change the job's info&send SIGCONT to the process group
          if fg job, wait for the end of fg job*/
        sigprocmask(SIG_BLOCK, &mask_all, &prev_one); 
        if(bg_fg == BG){
            switch(job->state){
                case ST:
    ...
    /*kill job
      npflag: 0 if jid or pid is negetive
              1 if jid or pid is positive*/
    if (tok.builtins == BUILTIN_KILL){
        ...
        /*jid, if negetive, then change it to positive*/
        if(tok.argv[1][0] == '%'){
        ...
        /*pid, if negetive, then change it to positive*/
        else{
            int _pid = atoi(tok.argv[1]);
            if(_pid < 0){
                npflag = 0;
                _pid = -_pid;
            }
            job = getjobpid(job_list, _pid);
            if(job == NULL){
                if(npflag){
                    fprintf(stdout, "(%d): No such process\n", _pid);
                }else{
    ...
    /*nohup
      block SIGCHLD SIGINT SIGTSTP when forking to avoid race conditions
      additionally, block SIGHUP the whole time*/
    if (tok.builtins == BUILTIN_NOHUP)
    {
        /*block SIGHUP,SIGCHLD,SIGINT,SIGTSTP*/
        sigprocmask(SIG_BLOCK, &mask_hup, &prev_one);
        sigprocmask(SIG_BLOCK, &mask_three, &prev_hup);
        pid = fork();
        if (pid == 0)
        {
            /*child process
              release the block of SIGCHLD SIGINT SIGTSTP, but still block SIGHUP*/
            sigprocmask(SIG_SETMASK, &prev_hup, NULL);
            /*seperate child process from shell's process group*/
            setpgid(0, 0);
            /*I/O redirection*/
            int fdout,fdin;
            if(tok.outfile == NULL)
        ...
        {
            /*parent process
              block all signals in case addjob gets interrupted*/
            ...
            else/*release the block of SIGCHLD,INT,TSTP so that the SIGCHLD signal 
                  can be collected*/
    ...
    if (pid == 0)
    {   /*child process
          release the block of SIGCHLD SIGINT SIGTSTP*/
        sigprocmask(SIG_SETMASK, &prev_one, NULL);
        /*seperate child process from shell's process group*/
        setpgid(0, 0);
        /*I/O redirection*/
    ...
    else
    {
        ...
        else/*wait for the end of fg job and collect it*/
            while(fgpid(job_list))
                sigsuspend(&prev_one);
        sigprocmask(SIG_SETMASK, &prev_one, NULL);
    }
...
sigchld_handler(int sig) 
{
    ...
    sigprocmask(SIG_BLOCK, &mask_all, &prev_one);
    /*print the message first and
      if the job is collected because it is stopped, set its state.
      if the job is collected because it is teminated, delete it.*/
